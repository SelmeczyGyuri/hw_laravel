...
<div class="form-back-link">
    <a href="{{ route('cars.show', $car->id) }}" class="btn btn-secondary">← Vissza a részletekhez</a>
</div>

<h1>
    <span class="title-bar" aria-hidden="true"></span>
    Szerkesztés: {{ $car->name }}
</h1>
...
<form action="{{ route('cars.update', $car->id) }}" method="post" class="race-form">
    @csrf
    @method('PUT')
        ...
        <div class="form-grid">
            ...
            <div class="form-field">
                ...
                <input type="text" name="toy_code" id="toy_code"
                       value="{{ old('toy_code', $car->toy_code) }}"
                       autocomplete="off">
            </div>
        </div>
    </div>
                ...
                <select name="year_id" id="year_id">
                    @foreach($years as $year)
                        <option value="{{ $year->id }}"
                            {{ old('year_id', $car->year_id) == $year->id ? 'selected' : '' }}>
                            {{ $year->year }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="series_id" id="series_id">
                    @foreach($series as $serie)
                        <option value="{{ $serie->id }}"
                            {{ old('series_id', $car->series_id) == $serie->id ? 'selected' : '' }}>
                            {{ $serie->series }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="designer_id" id="designer_id">
                    @foreach($designers as $designer)
                        <option value="{{ $designer->id }}"
                            {{ old('designer_id', $car->designer_id) == $designer->id ? 'selected' : '' }}>
                            {{ $designer->designer }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="form-section">
        ...
        <div class="extras-chips" id="extrasChips">
            @foreach ($extras as $extra)
                @php
                    $selectedExtras = old('extras', $car->extras->pluck('id')->toArray());
                @endphp
                <label class="chip {{ in_array($extra->id, $selectedExtras) ? 'chip--active' : '' }}">
                    <input type="checkbox"
                           name="extras[]"
                           value="{{ $extra->id }}"
            ...
            <label class="toggle-label" for="isPacked">
                ...
                <span class="toggle-wrap">
                    <input type="checkbox" name="isPacked" id="isPacked"
                           class="toggle-input"
                           {{ old('isPacked', $car->isPacked) ? 'checked' : '' }}>
        ...
        <div class="form-img-row">
            <div class="form-field form-field--grow">
                ...
                <input type="text" name="img_url" id="img_url"
                       value="{{ old('img_url', $car->img_url) }}"
                       autocomplete="off">
            </div>
            <div class="img-preview-wrap">
                <div class="img-preview" id="imgPreview">
                    @if(old('img_url', $car->img_url))
                        <img src="{{ old('img_url', $car->img_url) }}"
                             alt="{{ $car->name }}"
                             id="imgPreviewTag">
                    @else
    ...
    <div class="form-submit-row">
        <a href="{{ route('cars.show', $car->id) }}" class="btn btn-secondary">Mégsem</a>
